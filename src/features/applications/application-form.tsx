    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      {step === 4 && (
        <Section title="Invoice / LC / Terms">
          <Grid cols={3}>
            <Field label="Invoice No (blank = auto EXP)" {...register("invoice_no")} />
      ...
      {step === 7 && (
        <Section title="Shipping">
          <Grid cols={2}>
            <SelectField label="Port of Loading" value={watched.port_loading_text} onChange={(v) => setValue("port_loading_text", v)} options={(ports ?? []).map((p) => ({ value: p.name, label: p.name }))} />
            <SelectField label="Port of Discharge" value={watched.port_discharge_text} onChange={(v) => setValue("port_discharge_text", v)} options={(ports ?? []).map((p) => ({ value: p.name, label: p.name }))} />
      ...
      {step === 8 && (
        <Section title="Certificates / Annexure extras">
          <Grid cols={2}>
            ...
            <Field label="Examination Date" {...register("examination_date")} />
      ...
      {step === 9 && (
        <Section title="Review">
          <dl className="grid md:grid-cols-2 gap-2 text-sm">
            ...
            <div><span className="gov-label">Total</span><div className="font-semibold tabular-nums">{totalAmount.toFixed(2)} {watched.invoice_currency}</div></div>
